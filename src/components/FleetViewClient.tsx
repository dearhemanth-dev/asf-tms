...
export default function FleetViewClient({ role, immersive = false, viewMode = "map" }: FleetViewClientProps) {
  ...
  const hasMappableCoordinates = useCallback((vehicle: Vehicle) => {
    if (typeof vehicle.longitude !== "number" || typeof vehicle.latitude !== "number") return false;
    if (!Number.isFinite(vehicle.longitude) || !Number.isFinite(vehicle.latitude)) return false;
    if (vehicle.longitude < -180 || vehicle.longitude > 180) return false;
    if (vehicle.latitude < -90 || vehicle.latitude > 90) return false;
    return true;
  }, []);
  ...
  const filteredSourceVehicles = useMemo(
    () => ({
      all: vehicles,
    }),
    [vehicles]
  ...
  const haulingNoGpsAssets = useMemo(() => {
    const toAssetNumber = (vehicle: Vehicle) => {
      const normalizedTruck = String(vehicle.truckNo ?? "").trim();
      if (normalizedTruck) return normalizedTruck;
      const normalizedId = String(vehicle.id ?? "").trim();
      return normalizedId || "Unknown asset";
    };

    const uniqueSorted = (vehiclesForBucket: Vehicle[]) => {
      const deduped = new Set(vehiclesForBucket.map(toAssetNumber));
      return Array.from(deduped).sort((a, b) => a.localeCompare(b));
    };

    const base = vehicles.filter((vehicle) => !vehicle.atHome && !hasMappableCoordinates(vehicle));

    return {
      idle: uniqueSorted(base.filter((vehicle) => vehicle.status === "idle")),
      lessThan6: uniqueSorted(
        base.filter((vehicle) => vehicle.status === "moving" && (vehicle.mph ?? 0) > 0 && (vehicle.mph ?? 0) < 6)
      ),
      sixTo25: uniqueSorted(
        base.filter((vehicle) => vehicle.status === "moving" && (vehicle.mph ?? 0) >= 6 && (vehicle.mph ?? 0) <= 25)
      ),
      above25: uniqueSorted(base.filter((vehicle) => vehicle.status === "moving" && (vehicle.mph ?? 0) > 25)),
    };
  }, [vehicles, hasMappableCoordinates]);

  const summarizeNoGpsAssets = useCallback((assets: string[]) => {
    const preview = assets.slice(0, 2).join(", ");
  ...
  const haulingMenuCounts = useMemo(
    () => ({
      repairs: filteredSourceVehicles.all.filter((vehicle) => !vehicle.atHome && vehicle.status === "alert").length,
      idle: filteredSourceVehicles.all.filter((vehicle) => !vehicle.atHome && vehicle.status === "idle").length,
      lessThan6: filteredSourceVehicles.all.filter(
        (vehicle) => !vehicle.atHome && vehicle.status === "moving" && (vehicle.mph ?? 0) > 0 && (vehicle.mph ?? 0) < 6
      ).length,
      sixTo25: filteredSourceVehicles.all.filter(
        (vehicle) => !vehicle.atHome && vehicle.status === "moving" && (vehicle.mph ?? 0) >= 6 && (vehicle.mph ?? 0) <= 25
      ).length,
      above25: filteredSourceVehicles.all.filter(
        (vehicle) => !vehicle.atHome && vehicle.status === "moving" && (vehicle.mph ?? 0) > 25
      ).length,
    }),
    [filteredSourceVehicles]
  );

  const visibleVehicles = useMemo(() => {
    const sourceVehicles = filteredSourceVehicles.all;

    if (openChipMenu === "home") {
      return sourceVehicles.filter((vehicle) => Boolean(vehicle.atHome));
    }

    if (haulingFilter !== "All") {
      if (haulingFilter === "Repairs") {
        return sourceVehicles.filter((vehicle) => !vehicle.atHome && vehicle.status === "alert");
      }

      if (haulingFilter === "Idle") {
        return sourceVehicles.filter((vehicle) => !vehicle.atHome && vehicle.status === "idle");
      }
      ...
      if (haulingFilter === "Over 25 mph") {
        return sourceVehicles.filter((vehicle) => !vehicle.atHome && vehicle.status === "moving" && (vehicle.mph ?? 0) > 25);
      }
    }

    return sourceVehicles;
  }, [filteredSourceVehicles, haulingFilter, openChipMenu]);
      ...
      <div className="pointer-events-auto absolute left-0 top-full z-40 mt-0 w-44 rounded-xl border border-slate-700 bg-slate-950/95 p-0 shadow-[0_10px_28px_rgba(0,0,0,0.5)] backdrop-blur-sm">
        <ul className="space-y-0">
          {CHIP_MENU_ITEMS[menu].map((item) => {
            ...
            const haulingNoGpsAssetsForItem =
              menu === "hauling"
                ? item === "Idle"
                  ? haulingNoGpsAssets.idle
                  : item === "Under 6 mph"
                    ? haulingNoGpsAssets.lessThan6
                    : item === "6 to 25 mph"
                      ? haulingNoGpsAssets.sixTo25
                      : item === "Over 25 mph"
                        ? haulingNoGpsAssets.above25
                        : null
                : null;
            const shouldShowNoGpsHint = Boolean(haulingNoGpsAssetsForItem && haulingNoGpsAssetsForItem.length > 0);
            const noGpsSummary = shouldShowNoGpsHint ? summarizeNoGpsAssets(haulingNoGpsAssetsForItem as string[]) : "";
            const itemCount =
              menu === "hauling"
                ? item === "Repairs"
            ...
            return (
              <li key={item}>
                ...
                {shouldShowNoGpsHint && (
                  <div
                    className="px-2 pb-1.5 text-[10px] font-medium leading-tight text-slate-500"
                    title={`No GPS assets: ${(haulingNoGpsAssetsForItem as string[]).join(", ")}`}
                  >
                    No GPS: {noGpsSummary}
                  </div>
                )}
